...
export function registerSearchCatalogItemsTool(server: McpServer, client: ServiceNowClient): void {
  server.tool(
    ...
    async ({ query, catalogSysId, categorySysId, limit }) => {
      ...
      if (items.length === 0) {
        // Emit structuredContent so the catalog-browse widget mounts and
        // renders its own empty state (showing the query) instead of leaving
        ...
        return {
          ...
          structuredContent: { query, found: 0, items: [] }
        };
      }
      ...
      // UI narration ("shown above as cards…") that just duplicates the widget.
      return {
        content: [
          {
            type: "text" as const,
